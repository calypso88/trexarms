package com.calypso88.trexarms {

	/**
	 *	Flash 10.0 ◆ Actionscript 3.0
	 *	Copyright ©2010 Rob Sampson
	 *	www.calypso88.com
	 */
	...
	//-------------------------------------
		...
		import com.calypso88.trexarms.Pulsar;
		import flash.utils.Dictionary;

	/**
	 *  Chaingun is a replacement for the event model of application
	 *  communication. Notifications in Chaingun are still keyed off of
	 *  strings, but messaging is done on a broadcast model so listeners do
	 *  not need to know who the sender is.
	 * 
	 *  <p>Callbacks registered to Chaingun must accept one payload parameter,
	 ...
	 * 
	 *  <p>One important thing to note when using Chaingun is that the data
	 ...
	 *  been alerted yet.</p>
	 * 
	 *  <p>This version of Chaingun uses four tiers of priority. All single-use
	 ...
	 *  
	 *  <p>Chaingun also has dependency chaining. When a callback is registered
	...
	public final class Chaingun {
		...
		//--------------------------------------
			...
			//  Holds the singleton
			private static const instance:Chaingun = new Chaingun();
		...
		//--------------------------------------
			...
			public function Chaingun(){
				...
				if(!instance) trace('[T-Rex Arms]: Chaingun initialized, v.' + VERSION);
			}
		...
		//--------------------------------------
			...
			/**  Whether we have asked for onTick callbacks from Pulsar */
...
//				if(populateImmediately && state in STATE_ARCHIVE) setState(state, STATE_ARCHIVE[state]);
			...
			public function addSingleUseListenerWithDependencies(state:String, callback:Function, dependentStates:Array, populateImmediately:Boolean = true):void{
				...
				if(dependentStates.length == 0){
					...
					//  this object will hook back into Chaingun until it's 
...
//				if(populateImmediately && state in STATE_ARCHIVE) setState(state, STATE_ARCHIVE[state]);
			...
			public function setState(state:String, data:*, processImmediately:Boolean = false):*{
				...
				if(_settingState && !processImmediately){
					...
					if(!_ticking) Pulsar.registerCallback(onTick);				//  we need to be called in the future - so register for updates
					_ticking = true;
					return;
				}
...
//						callback(data);
		...
		//--------------------------------------
			...
			/**
			 *  Called each tick by Pulsar. If their are queued states, 
			...
			private function onTick():void{
				...
				if(PENDING_STATE_KEYS.length == 0){
					...
					Pulsar.unregisterCallback(onTick);
					_ticking = false;
				}
			}
...
//----------------------------------------------
	...
	import com.calypso88.trexarms.Chaingun;
	
	/**  private class for managing dependencies inside Chaingun. */
		...
		//--------------------------------------
			...
			private function listenForNextBlocker(...args):void{
				if(_blockers.length){
					...
					Chaingun.addSingleUseListener(_blockers.pop(), listenForNextBlocker, true);
				} else {
